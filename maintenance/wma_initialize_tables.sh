...
do
  ...
  echo "DROP TABLE coord_${lang}; CREATE TABLE IF NOT EXISTS coord_${lang} (coord_id INT(8) PRIMARY KEY AUTO_INCREMENT, page_id INT(8), lat FLOAT NOT NULL, lon FLOAT NOT NULL, style TINYINT, weight FLOAT, scale INT(4), title VARBINARY(255), globe SMALLINT UNSIGNED NOT NULL);" | mysql -h${server} ${db}
done
