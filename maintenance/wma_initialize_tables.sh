...
do
  eval "cat <<EOF
  $(<wma_storedprocedures.sql)
  " 2> /dev/null | mysql -h${server} ${db} 
...
echo "CREATE TABLE IF NOT EXISTS coord_commons (coord_id INT(8) PRIMARY KEY AUTO_INCREMENT, page_id INT(8), lat FLOAT NOT NULL, lon FLOAT NOT NULL, heading FLOAT, iw INT(11), ih INT(11), style TINYINT, weight FLOAT, scale INT(4), title VARBINARY(255), globe SMALLINT UNSIGNED NOT NULL, bad BOOLEAN NOT NULL);" | mysql -h${server} ${db}
