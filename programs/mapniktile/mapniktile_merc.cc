...
{    
    ...
    datasource_cache::instance()->register_datasources("/opt/ts/lib/mapnik/input"); 
    freetype_engine::register_font("/usr/share/fonts/truetype/ttf-dejavu/DejaVuSans.ttf");
    
    //Map m( 256, 256, "+proj=merc +datum=WGS84 +k=1.0 +units=m +over +no_defs" );
    //Map m( 256, 256, "+proj=merc +a=128 +b=128 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +no_defs +over" );

    double bx1,by1,bx2,by2;
    int mw = 256, mh =256;
    projection proj( "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +no_defs" );
    if( argc == 8 )
    {

        // lat1 lat2 lon1 lon2 w flags filename
        bx1 = atof( argv[3] );
        by1 = atof( argv[1] );
        bx2 = atof( argv[4] );
        by2 = atof( argv[2] );
        ...
        proj.forward( bx1, by1 );
        proj.forward( bx2, by2 );

        double ar = ( bx1 - bx2 ) / ( by1 - by2 );
        if( ar > 1 ) 
            { mw = w; mh = int(w/ar); }
        else
            { mw = int(w*ar); mh = w; }
    ...
    Map m( mw, mh, "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +no_defs" );
...
*/    
    ...
    int xx;
    std::string base_dir = "merc/vmap0/";
    ...
    if( argc == 2 )
    {
    	...
    	//
    	sprintf( filename, "/tmp/wikiminiatlas.tile.merc%d.pid", z );
    	fp = fopen( filename, "wt" );
    	fprintf( fp, "%d", getpid() );
    	fclose( fp );

	    sprintf( filename, "/tmp/wikiminiatlas.tile.merc%d.fifo", z );
    	...
    	while(1)
    	{
	    	if( fgets( readbuf, 800, fp ) != 0 )
		    {
    			...
    			{
	    			sprintf( tilefile, "mapnik/merc/vmap0/%d/%d/%d.png", z, x, y );
    ...
    else if( argc == 8 )
    {
        ...
        save_to_file<ImageData32>( buf.data(), argv[7], "png" );
