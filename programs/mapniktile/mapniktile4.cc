...
{    
    ...
    */
    ...
    rule lake2poly_rule;
    lake2poly_rule.set_filter(parse_expression("[waterway] = 'riverbank' or [waterway] = 'dock' or [natural] = 'bay' or [natural] = 'water'"));
    ...
    rule builtup2_rule;
    builtup2_rule.set_filter(parse_expression("[landuse] = 'residential' or [landuse] = 'retail' or [landuse] = 'commercial' or [landuse] = 'industrial'"));
    ...
    rule greenuse_rule;
    greenuse_rule.set_filter(parse_expression("[landuse] = 'orchard' or [landuse] = 'cemetary' or [landuse] = 'meadow' or [landuse] = 'village_green' or [landuse] = 'forrest' or [landuse] = 'recreation_ground' or [leisure] = 'dog_park' or [leisure] = 'garden' or [leisure] = 'park' or [leisure] = 'pitch' or [leisure] = 'stadium'"));
...
*/
	...
	while(1)
	{
	  if( fgets( readbuf, 800, fp ) != 0 )
	  {
		...
		std::cout << "already created file " << tilefile << std::endl;
              ...
              save_to_file<image_data_32>( buf.data(), tilefile, "png" );
	...
	if( x >= 3*(1<<z) ) xx = x - 6*(1<<z); 
        ...
        save_to_file<image_data_32>( buf.data(), argv[4], "png" );
...
//	return 1;
    	...
    	for( int z = 6; z<=6; z++)
    		for( int x = 227; x < 6*(1<<z); x++ )
				...
				agg_renderer<image_32> ren(m,buf);
				ren.apply(); 
				save_to_file<image_data_32>( buf.data(), fname.str(), "png" );
			}
